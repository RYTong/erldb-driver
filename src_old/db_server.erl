...
init_default(ConfigName) ->
    ConnArg = get_db_config(ConfigName),
    init_default(ConfigName, ConnArg).

%% @doc Read config and init default connection pool.
%% @spec init_default(ConfigName::atom(), ConnArg::tuplelist()) -> ok | throw()
init_default(ConfigName, ConnArg) ->
...
connect(Name, Args) ->
    case connect(Args) of
        {ok, Instance} ->
            ...
            DbType = db_util:drv_name(proplists:get_value(driver, Args)),
...
get_db_config(ConfigName) ->
    case get_config_path() of
        {error, _Err} ->
            Config = ewp_conf_util:get_ewp_conf_value(databases, []),
...
get_config_path() ->
    ...
    case filelib:is_file(ConfigName) of
        ...
        false ->
            case code:lib_dir(?DB_APP_NAME, list_to_atom(?DB_CONFIG_PATH)) of
                {error, Err} ->
                    {error, Err};
                Path ->
                    Path
...
get_db_config(ConfigName, ConfigPath) ->
    ...
    case file:consult(DBConfigFile) of
        {error, Err} ->
            throw(lists:concat(["Can not find ", ?DB_CONFIG_FILE, ": ", Err]));
...
get_tables(Name, DbName, ?MYSQL_DB) ->
    select({information_schema,'.',columns}, {table_schema,'=',DbName},
        [{fields,table_name}, {distinct,true}, {db_name, Name}]);
get_tables(Name, _DbName, ?ORACLE_DB) ->
    select(user_tables, [], [{fields, table_name},
        {distinct, true}, {db_name, Name}]);
get_tables(Name, _DbName, ?SYBASE_DB) ->
    select(sysobjects, {type, '=', "U"}, [{fields, name},
        {distinct, true}, {db_name, Name}]);
get_tables(Name, _DbName, ?DB2_DB) ->
    {ok, [[CurrentSchema]]} = execute_sql("VALUES CURRENT SCHEMA", [{db_name, Name}]),
    {ok, Rows} = execute_sql("SELECT DISTINCT TABSCHEMA, TABNAME FROM SYSCAT.TABLES", [{db_name, Name}]),
    {ok, lists:foldl(
...
get_tables(Name, _DbName, ?INFORMIX_DB) ->
    execute_sql("select tabname from systables where tabid > 99 and tabtype = 'T'",
        [{db_name, Name}]).
...
get_fields(Name, _DbName, TableName, ?ORACLE_DB) ->
    execute_sql("select column_name from user_tab_columns where table_name='"
        ++ TableName ++ "' order by column_id", [{db_name, Name}]);
get_fields(Name, _DbName, TableName, ?SYBASE_DB) ->
    execute_sql("select name from syscolumns where id=object_id('"
        ++ TableName ++ "')", [{db_name, Name}]);
get_fields(Name, _DbName, TableName, ?DB2_DB) ->
    {ok, [[CS]]} = execute_sql("VALUES CURRENT SCHEMA", [{db_name, Name}]),
    {TabSchema, TabName} =
        case string:tokens(TableName, ".") of
            [TS, TN] -> {TS, TN};
            [TN]     -> {CS, TN}
        end,
    execute_sql("SELECT COLNAME from SYSCAT.COLUMNS where TABSCHEMA='"  ++ TabSchema
        ++ "' AND TABNAME='" ++ TabName ++ "' ORDER BY COLNO", [{db_name, Name}]);
get_fields(Name, _DbName, TableName, ?INFORMIX_DB) ->
    {ok, [[TableId]]} = execute_sql("select tabid from systables where tabname = '"
        ++ TableName ++ "'", [{db_name, Name}]),
    execute_sql("select colname from syscolumns where tabid = "
        ++ integer_to_list(TableId) ++ " order by colno", [{db_name, Name}]).
%select colname from syscolumns where tabid = xxx order by colno
