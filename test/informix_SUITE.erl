...
init_per_testcase(TestCase, Config) ->
    ...
    {ok, _} = db_api:execute_sql("DROP TABLE IF EXISTS " ++ Table),
...
execute_sql(Config) ->
    ...
    {ok, 1} = db_api:execute_sql("insert into " ++ Table ++ " (fvarchar, ffloat, fint) values('denglf', 24, 123)"),
    {ok, [["denglf"]]} = db_api:execute_sql("select fvarchar from " ++ Table ++ " where fint = 123"),
    {ok, 1} = db_api:execute_sql("update " ++ Table ++ " set ffloat=22.2 where fvarchar = 'denglf'"),
    {ok, [[22.2, 123]]} = db_api:execute_sql("select ffloat,fint from " ++ Table ++ " where fint = 123"),
    {ok, 1} = db_api:execute_sql("delete from " ++ Table ++ " where fvarchar = 'denglf'"),
    {ok, []} = db_api:execute_sql("select * from " ++ Table ++ " where fint = 123"),
...
sql_api(Config) ->
    ...
    {ok, [[1]]} = db_api:select(Table, [], [{fields, [fserial]}]),
    {ok, []} = db_api:select(Table, {fchar, '=', undefined}),
    Num = Int + Small,
    {ok, 1} = db_api:update(Table, [{fint, {fint, '+', fsmall}}], {fserial, '=', 1}),
    {ok, [[Num]]} = db_api:select(Table, {fserial, '=', 1}, [{fields, [fint]}]),

    CNString = "好的，亲！",
    {ok, 1} = db_api:insert(Table, [{fvarchar, CNString}]),
    {ok, [[CNString]]} = db_api:select(Table, {fserial, '=', 2}, [{fields, [fvarchar]}]),
    {comment, "Test of api of insert, delete, update and select."}.

expression(Config) ->
    Table = list_to_atom(?config(table_name, Config)),
    Table2 = list_to_atom(?config(table_name, Config) ++ "2"),
    {ok, 1} = db_api:insert(Table, [{fvarchar, "group1"}, {fint, 1}]),
    {ok, 1} = db_api:insert(Table, [{fvarchar, "group1"}, {fint, 3}]),
    {ok, 1} = db_api:insert(Table, [{fvarchar, "group2"}, {fint, 1}]),
    {ok, 1} = db_api:insert(Table, [{fvarchar, "group2"}, {fint, 3}]),
    {ok, [[3]]} = db_api:select(Table, [], [{fields, [{max, [fint]}]}]),
    {ok, [["group1", 3], ["group2", 3]]} = db_api:select(Table, [],
        [{fields, [fvarchar, fint]},
            {extras, [{group, [fvarchar, fint]}, {having, {fint, in, [3, 4]}}, {order, {fvarchar, asc}}]}
        ]),

    {ok, _} = db_api:execute_sql("drop table if exists " ++ atom_to_list(Table2)),
    {ok, _} = db_api:execute_sql("create table " ++ atom_to_list(Table2) ++ "(
        fvarchar varchar(20) default NULL,
        fint integer default NULL)"),
    {ok, 1} = db_api:insert(Table2, [{fvarchar, "group3"}, {fint, 1}]),
    {ok, 1} = db_api:insert(Table2, [{fvarchar, "group3"}, {fint, 3}]),
    ...
    {ok, [["group1", "group3"], ["group2", "group3"]]} =
        db_api:select(JoinExpr, [], [{fields, Fields}, {distinct, true}, {extras, [OrderByExpr]}]),

    AndExpr = {'and', [{fint, '>', 1}, {fint, '<', 5}]},
    LikeExpr = {fvarchar, like, "%1"},
    {ok, [["group1", 3], ["group2", 1], ["group2", 3]]} =
        db_api:select(Table, {AndExpr, 'or', {'not', LikeExpr}}, [{fields, [fvarchar, fint]}]),
...
datetime(Config) ->
    ...
    {ok, 1} = db_api:insert(Table, [{fdate, {date, Date}}]),
    {ok, 1} = db_api:execute_sql("insert into " ++ ?config(table_name, Config) ++ " (fdate) values ('" ++ DateStr ++ "')"),
    DateRes = {ok, lists:duplicate(2, [{date, Date}])},
    DateRes = db_api:select(Table, {fdate, '=', {date, Date}}, [{fields, [fdate]}]),
    ...
    {ok, 1} = db_api:insert(Table, [{fdatetime, Datetime}]),
    {ok, 1} = db_api:execute_sql("insert into " ++ ?config(table_name, Config) ++ " (fdatetime) values ('" ++ DatetimeStr ++ "')"),
    DatetimeRes = {ok, lists:duplicate(2, [{datetime, {Date,{Hour, Min, Sec}}}])},
    DatetimeRes =
        db_api:select(Table, {fdatetime, '=', Datetime}, [{fields, [fdatetime]}]),
    ...
    {ok, 1} = db_api:insert(Table, [{finterval1, Interval1}]),
    {ok, 1} = db_api:execute_sql("insert into " ++ ?config(table_name, Config) ++ " (finterval1) values ('" ++ Interval1Str ++ "')"),
    {ok, [[{datetime, {{Year, Month, _},_}}], [{datetime, {{Year, Month, _},_}}]]} =
        db_api:select(Table, {finterval1, '=',Interval1}, [{fields, [finterval1]}]),
    ...
    {ok, 1} = db_api:insert(Table, [{finterval2, Interval2}]),
    {ok, 1} = db_api:execute_sql("insert into " ++ ?config(table_name, Config) ++ " (finterval2) values ('" ++ Interval2Str ++ "')"),
    {ok, [[{datetime, {{_, _, Day}, {Hour, Min, {"number", SecStr}}}}], [{datetime, {{_, _, Day}, {Hour, Min, {"number", SecStr}}}}]]} =
        db_api:select(Table, {finterval2, '=',Interval2}, [{fields, [finterval2]}]),
...
lob(Config) ->
    ...
    String = "大对象类型数据",
    {ok, 1} = db_api:insert(Table, [{fbyte, Binary}, {fblob, {"blob", Binary}}, {ftext, {"text", String}}, {fclob, {"clob", String}}]),
    {ok, [[Binary, Binary, String, String]]} =
        db_api:select(Table, [], [{fields, [fbyte, fblob, ftext, fclob]}]),
...
long_sql(Config) ->
    try
        ...
        {ok, 1} = db_api:execute_sql(LongData, [{timeout, 15*1000}]),
        Num = integer_to_list(N) ++ ".0",
        {ok, [[{"number", Num}]]} =
            db_api:select(list_to_atom(Table), [], [{fields, [{count, ['*']}]}]),
...
trans(Config) ->
    ...
    {atomic, ok} = db_api:transaction(
        fun() ->
                ...
                {ok, [[2], [1]]} = db_api:select(Table, {fint, '>', 0},
                                                 [{fields, fint},
                                                  {extras, [{order, {fint, desc}}]}]),
                {ok, [[{"number", "2.0"}, 44.25]]} = db_api:select(Table, [],
    ...
    {aborted, {{error, _Err}, {rollback_result, {ok, "ROLLBACK"}}}} = db_api:transaction(
        fun() ->
                Table = TableName,
                {ok, 1} = db_api:insert(Table, [{fvarchar, "trans test1"}]),
                db_api:insert(Table, [{fdate, "2013/5/5"}, {fvarchar, "trans test2"}]),
                db_api:insert(Table, [{fvarchar, "trans test3"}])
        end),
...
change_pool_size(Config) ->
    ...
    ok = db_api:add_pool(PoolId, lists:keyreplace(poolsize, 1, ConnArg, {poolsize, 2})),
    2 = db_api:get_pool_info(PoolId, size),
    ok = db_api:increment_pool_size(PoolId, 2),
    4 = db_api:get_pool_info(PoolId, size),
    ok = db_api:decrement_pool_size(PoolId, 3),
    1 = db_api:get_pool_info(PoolId, size),
    ok = db_api:decrement_pool_size(PoolId, 3),
...
stored_procedure(Config) ->
    ...
    {ok, _} = db_api:execute_sql("create procedure " ++ Procedure ++ "() begin " ++ Sql ++ " ; end end procedure"),
    {ok, _} = db_api:execute_sql("call " ++ Procedure ++ "();"),
    {ok, [["procedure", 123]]} = db_api:select(list_to_atom(Table), [], [{fields, [fvarchar, fint]}]),
    {ok, _} = db_api:execute_sql("drop procedure " ++ Procedure),
...
stmt(Config) ->
    ...
    PUpdate = p_update,
    PSelect = p_select,
    ok = db_api:prepare(PInsert, "insert into " ++ Table ++ " (fvarchar, fint) values(?, ?)"),
    ok = db_api:prepare(PUpdate, "update " ++ Table ++ " set fvarchar=? where fint = ?"),
    ok = db_api:prepare(PSelect, "select fvarchar from " ++ Table ++ " where fint=?"),
    {error, _} = db_api:prepare(PUpdate, "update " ++ Table ++ " set fvarchar=?"),
    {ok, 1} = db_api:prepare_execute(PInsert, ["stmt test", 1]),
    {ok, [["stmt test"]]} = db_api:prepare_execute(PSelect, [1]),
%        db_api:select(list_to_atom(Table), {fint, '=', 1}, [{fields, [fvarchar]}]),
    {ok, 1} = db_api:prepare_execute(PUpdate, ["test stmt", 1]),
    {ok, [["test stmt"]]} = db_api:prepare_execute(PSelect, [1]),
%        db_api:select(list_to_atom(Table), {fint, '=', 1}, [{fields, [fvarchar]}]),
